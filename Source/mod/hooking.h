...
// This find the correct static function signature for a given class method
// usage: typename MethodTrait<
...
private:
	TArray<HookFunc>* functionList;
	size_t handlerPtr = 0;
	std::function<HookType> function;
	
	bool forwardCall = true;
	Result result;

public:
    CallScope(TArray<HookFunc>* functionList, std::function<HookType> function) : functionList(functionList), function(function) {}
...
// Const Method
template <typename Result, typename C, typename... Args>
struct CallScope<Result(C::*)(Args...) const> : public CallScope<Result(*)(C const*, Args...)> {};
...
template <typename R, typename C, typename... A, typename NewTarget>
struct RetargetFuncType<R(C::*)(A...) const, NewTarget> {
public:
    typedef R(NewTarget::*Value)(A...) const;
};
...
//*/ Hook invoker for const member functions
...
{
public:
    typedef CallScope<R(*)(C const*, A...)> ScopeType;
    // mod handler function
    typedef void HandlerSignature(ScopeType&, C const*, A...);
    typedef typename HandlerAfterFunc<R, C const*, A...>::Value HandlerSignatureAfter;
    typedef R HookType(C const*, A...);
    typedef R ReturnType;
...
public:
    // Trampoline function using internal implementation of functions returning class/union type
    // Used to re-oder function parameter order.
    // When calling *static* functions returning by value, 
    // first argument is pointer to memory allocated by caller for storing return value.
    // But for calling *member* functions, first argument is always this pointer, so 

    static std::function<typename ScopeType::HookType> TrampolineFunctionCall(R* outReturnValue) {
        return [&outReturnValue](C const* self, A... args) -> R {
            (reinterpret_cast<R*(*)(C const*, R*, A...)>(functionPtr))(self, outReturnValue, args...);
            return *outReturnValue;
        }; 
    }
    ...
    //Methods which return class/struct/union by value have out pointer inserted
    //as first parameter after this pointer, with all arguments shifted right by 1 for it
    static R* applyCallUserTypeByValue(C const* self, R* outReturnValue, A... args) {
        auto fun = TrampolineFunctionCall(outReturnValue);
        ScopeType scope(handlersBefore, fun);

        scope(self, args...);
        for (HandlerAfter& handler : *handlersAfter) handler(scope.getResult(), self, args...);
        // We always return outReturnValue, so copy our result to output variable and return it
        // We do not need to do this anymore R* outReturnValue was set when the real function was called
        *outReturnValue = scope.getResult();
        return outReturnValue;
    }
    ...
    //If it were returning user type by value, first argument would be R*, which is incorrect - that's why we need separate
    //applyCallUserType with correct argument order
    static R applyCallScalar(C const* self, A... args) {
        ScopeType scope(handlersBefore, functionPtr);
        scope(self, args...);
        for (HandlerAfter& handler : *handlersAfter) handler(scope.getResult(), self, args...);
        return scope.getResult();
    ...
    //Call for void return type - nothing special to do with void
    static void applyCallVoid(C const* self, A... args) {
        ScopeType scope(handlersBefore, functionPtr);
        scope(self, args...);
        for (HandlerAfter& handler : *handlersAfter) handler(self, args...);
    }
...
template <typename R, typename C, typename... A, R(C::*PMF)(A...) const, typename TargetClass>
R(*HookInvoker<R(C::*)(A...) const, PMF, TargetClass>::functionPtr)(C const*, A...) = nullptr;
